    ...
    for(let i = 0, len = this.minions.length; i < len; i++){
      ...
      if(minion.active){
        ...
        if(!minion.active){
          ...
          this.particles.push(
            new Particle(minion.target_base.left, minion.target_base.top, minion.target_base.scale, minion.source_base.color)
            );
        }
      }
